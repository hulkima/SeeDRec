...
#         ipdb.set_trace()
    ...
    def scale_withminmax(self, args, index_list, time_list):
        min_list = np.min(time_list)
        max_list = np.max(time_list)
        if min_list == max_list:
            if args.reweight_version == "AllOne":
                scaled_array = np.ones(shape=len(time_list),dtype=np.float32)*args.w_max
            elif args.reweight_version == "AllLinear":
                scaled_array = np.linspace(args.w_min, args.w_max, len(time_list), dtype=np.float32)
            elif args.reweight_version == "MinMax":
                max_list = max_list + 1e3    
                scaled_array = normalization(time_list, args.w_min, args.w_max, min_list, max_list)
                return index_list, scaled_array
        else:
            scaled_array = normalization(time_list, args.w_min, args.w_max, min_list, max_list)

        if args.overlap_version == "LastOne":
            transform_index_array, transform_time_array = expand_lists_with_weights_np(index_list, scaled_array)
        elif args.overlap_version == "Sum":
            transform_index_array, transform_time_array = expand_lists_with_weights_np(index_list, scaled_array)
            transform_index_array, transform_time_array = transform_lists(transform_index_array, transform_time_array)   
        elif args.overlap_version == "Sum_with_Norm":
            transform_index_array, transform_time_array = expand_lists_with_weights_np(index_list, scaled_array)
            transform_index_array, transform_time_array = transform_lists(transform_index_array, transform_time_array)  
            transform_time_array = transform_time_array / np.max(transform_time_array)

#         ipdb.set_trace()
        corpus_target = np.zeros([args.catenum+1], dtype=np.float32)
        corpus_target[transform_index_array] = transform_time_array
    ...
    def generate_sample(self, args, model_diff, user_cate_train, user_time_train):
        corpus_target = self.scale_withminmax(args, user_cate_train, user_time_train)
        diff_matrix = self.p_sample(model_diff, th.tensor(corpus_target, device='cuda').unsqueeze(0), args.sampling_steps, args.sampling_noise).detach().squeeze()
        return diff_matrix
...
def normalization(time_list, min_value_static, max_value_static, min_value_list, max_value_list):
    time_list_std = (time_list - min_value_list) / (max_value_list - min_value_list)
    time_list_scaled = time_list_std * (max_value_static - min_value_static) + min_value_static
    return time_list_scaled

def transform_lists(index_list, time_list):
    mask = index_list[:, None] == np.unique(index_list)
    time_list_new = np.where(mask, time_list[:, None], 0).sum(axis=0)
    index_list_new = np.unique(index_list)

    sorted_indices = np.argsort(time_list_new)
    index_list_new = index_list_new[sorted_indices]
    time_list_new = time_list_new[sorted_indices]

    return index_list_new, time_list_new

def expand_lists_with_weights_np(lists, weights):
    A_np = np.array([item for sublist in lists for item in sublist])
    B_np = np.repeat(weights, [len(sublist) for sublist in lists])
...
def mean_flat_correct(tensor, max_dim):
    """
    Take the mean over all non-batch dimensions.
    """
#     ipdb.set_trace()
#     return tensor.mean(dim=list(range(1, len(tensor.shape))))
    return tensor.sum(dim=list(range(1, len(tensor.shape))))/max(tensor.shape[1], max_dim)